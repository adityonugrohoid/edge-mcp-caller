...
- `mcp/client.py` — Bridge: model JSON output → MCP tools/call protocol (14 tools across 2 servers)
...
- `docs/generation-standard.md` — Unified data generation standard for all 14 tools
- `docs/mcp-query-behavior-analysis.md` — v0.2 failure analysis and extraction rules
...
| # | Tool | Args (model output) | Version |
|---|------|---------------------|---------|
| 1 | `list_directory` | `path` | v0.1 |
| 2 | `read_file` | `path` | v0.1 |
| 3 | `search_files` | `path`, `pattern` | v0.1 |
| 4 | `write_file` | `path`, `content` | v0.2 |
| 5 | `create_directory` | `path` | v0.2 |
| 6 | `edit_file` | `path`, `old_text`, `new_text` | v0.3 |
| 7 | `move_file` | `source`, `destination` | v0.3 |
| 8 | `directory_tree` | `path` | v0.3 |

### Git MCP Server (6 tools)
| # | Tool | Args (model output) | Version |
|---|------|---------------------|---------|
| 9 | `git_status` | _(none — bridge injects repo_path)_ | v0.4 |
| 10 | `git_diff_staged` | _(none)_ | v0.4 |
| 11 | `git_commit` | `message` | v0.4 |
| 12 | `git_log` | `max_count` _(optional, integer)_ | v0.4 |
| 13 | `git_branch` | _(none)_ | v0.4 |
| 14 | `git_create_branch` | `branch_name`, `base_branch` _(optional)_ | v0.4 |
...
## v0.2 Dataset Stats (archived — see docs/generation-standard.md for current)
...
- **v0.1 data gen**: NVIDIA NIM API (llama-3.1-70b-instruct), overwrites train/eval each run (legacy)
- **v0.2 data gen**: Claude Code agents (Sonnet 4.6), accumulate-then-merge (archived — polluted data)
- **v0.3+ data gen**: Claude Code agents (Sonnet 4.6), extraction-only rules per `docs/generation-standard.md`
...
- Data split target: 70% clean, 15% messy, 15% disambiguation — all with extractable args only
